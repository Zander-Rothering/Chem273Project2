...
class ConcentrationProfile:
    #Class to define radial concentration profiles using Gaussian functions"""
...
class ChemotaxisSimulation:
    ...
    def plot_stacked_frequency_evolution(self, ax=None, bins=20, stack_offset=None):
        """Plot stacked radial frequency distribution with vertical offset"""
        if ax is None:
        ...
        for i, ((x_pos, y_pos), time) in enumerate(zip(self.position_snapshots, self.snapshot_times)):
            ...
            offset_counts = counts + (i * stack_offset)
            
            # Plot as filled area for better visibility
            ax.fill_between(bin_centers, i * stack_offset, offset_counts, 
                           color=colors[i], alpha=0.7, label=f't={time}')
            
            # Add line on top for clarity
            ax.plot(bin_centers, offset_counts, color=colors[i], 
...
def run_population_analysis():
    ...
    populations = [10, 100, 1000]  # Increased populations for better visualization
    gradient_types = {
...
def create_population_comparison_plots(results):
    ...
    # 3. Overlapping Frequency Evolution
